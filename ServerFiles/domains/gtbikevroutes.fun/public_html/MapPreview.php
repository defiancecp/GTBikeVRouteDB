...
<head>
	...
	<script src="AnimatedMarker.js" charset="utf-8"></script>


	<link rel="stylesheet" href="MPStyles.css">
	<style media="screen">
		.leafletContainer{ display:block; width:100%; height:100%; margin:0; background:#eee}
		.leaflet-container{ min-height:800px;}
...
<div class="leafletContainer">
	...
	<script>
		var map = L.map('map').setView([-19.08608811895865, -169.88037081188432], 13);
		let atlas =L.tileLayer('/tiles/Atlas/{z}/{x}/{y}.png', { maxZoom: 18, tms: false, attribution: 'Generated by QTiles' }).addTo(map);
		let road = L.tileLayer( '/tiles/Road/{z}/{x}/{y}.png', { maxZoom: 18, tms: false, attribution: 'Generated by QTiles' }).addTo(map);
		let sat  = L.tileLayer(  '/tiles/Sat/{z}/{x}/{y}.png', { maxZoom: 18, tms: false, attribution: 'Generated by QTiles' }).addTo(map);
		...
		let gpx = new L.GPX('/gpx/alamo_sea.gpx', {
			async       : true,
			marker_options: {
			...
			let hackosaurous = null;
			for(let a of Object.keys(gpx._layers)){ hackosaurous=gpx._layers[a]; break; }
			for(let a of Object.keys(hackosaurous._layers)){ hackosaurous=hackosaurous._layers[a]; break; }
			let marker = L.animatedMarker( hackosaurous._latlngs );
			map.addLayer(marker);
		}).addTo(map);
